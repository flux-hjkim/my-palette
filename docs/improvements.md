...
**Before**  
title 또는 description이 비어 있어도 SubColor가 저장될 수 있었음.

**After**  
저장 전 title과 description의 빈 값 여부를 검사하고, 값이 없으면 저장하지 않도록 처리.

**Reason**  
의미 없는 컬러 기록이 생성되지 않도록 하고, 사용자가 필수 입력값을 인지할 수 있게 하기 위함.

**Result**  
Add/Edit 저장 흐름의 데이터 안정성이 개선되었고, 빈 값 저장으로 인한 잘못된 데이터 생성을 방지할 수 있게 됨.
...
**Description**  
`handleSave`에서 `nextErrors` 객체를 생성하고, `Object.values().some()`으로 에러 여부를 확인한 뒤 저장 여부를 결정하도록 구현.

**Reason**  
Form 입력 UI와 실제 저장 로직을 분리하면서도, 저장 직전에 필수 입력값을 한 번 더 검증하기 위함.

**Result**  
Add/Edit 공용 Editor Page에서 동일한 validation 흐름을 사용할 수 있게 되었고, 저장 로직의 안정성이 개선됨.
