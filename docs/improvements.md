...
**Before**
상세 Modal을 열었을 때 배경 페이지가 함께 스크롤됨.

**After**  
상세 Modal이 열리면 `document.body.style.overflow = "hidden"`을 적용해 배경 스크롤을 막고, Modal이 닫히면 이전 스크롤 상태로 복구.
...
**Before**  
전체 컬러를 한 화면에 표시하더라도 색상만으로는
각 컬러가 어떤 기록인지 바로 파악하기 어려웠음.

**After**  
컬러 스티커의 기본 상태에는 컬러명을 표시하고,
hover 시 카테고리, 제목, keyword 정보를 확인할 수 있도록 구성.

**Reason**  
스티커 보드의 시각적 분위기는 유지하면서도
필요할 때 각 컬러 기록의 의미를 탐색할 수 있도록 하기 위함.

**Result**  
사용자는 전체 컬러의 분위기를 먼저 살펴보고,
관심 있는 스티커에서 기록 정보를 단계적으로 확인할 수 있음.

---

### 16. 컬러 추가와 분포 확인 도구 제공

**Before**  
Pieces of Me에서 전체 컬러를 확인할 수 있어도
새 컬러 추가와 카테고리별 기록 분포 확인을 위해 다른 화면을 탐색해야 했음.

**After**  
컬러를 추가할 카테고리 선택 기능과
카테고리별 컬러 개수·상대 비율을 표시하는 도구 영역을 추가.
모바일에서는 도구 영역을 접고 펼칠 수 있도록 구성해 스티커 보드 접근성을 유지.

**Reason**  
전체 컬러를 감상하는 흐름 안에서
기록 추가와 현재 컬러 분포 확인까지 이어질 수 있도록 하기 위함.

**Result**  
사용자는 Pieces of Me 화면에서 새 컬러를 추가하고,
카테고리별 기록 균형을 함께 확인할 수 있음.

---

### 17. 페이지 이동 시 스크롤 위치 초기화
...
### 18. 날짜 기반 컬러 기록 탐색 추가

**Before**  
사용자가 추가한 컬러를 카테고리별 화면이나 Pieces of Me에서 확인할 수 있었지만,
컬러가 언제 기록되었는지 시간의 흐름에 따라 살펴보기 어려웠음.

**After**  
SubColor의 `createdAt`을 기준으로 컬러를 날짜별로 분류하고,
기록이 존재하는 월을 월간 달력 형태로 탐색할 수 있는 Palette Log를 추가.

**Reason**  
컬러 기록을 단순 목록이 아니라 사용자의 취향과 관심사가 쌓여 온 흐름으로
확인할 수 있도록 하기 위함.

**Result**  
사용자는 기록이 존재하는 월을 이동하며 날짜별 컬러를 확인하고,
달력의 컬러를 선택해 기존 상세 Modal에서 수정·삭제할 수 있음.
...
### 13. createdAt 기반 Palette Log 파생 데이터 구성

**Description**  
각 카테고리의 SubColor를 하나의 배열로 변환하고,
`createdAt`에서 월과 날짜 key를 추출하여 기록 월 목록과 날짜별 컬러 배열을 생성.

**Reason**  
기존 `colorGroups` 데이터를 직접 변경하지 않고,
동일한 데이터를 달력 화면에 맞는 구조로 활용하기 위함.
