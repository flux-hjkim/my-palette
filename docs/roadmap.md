...
| v1.2.0  | Sticker Frame UI     | Pieces of Me 전용 스티커 UI 고도화      |
...
| v3.1.0  | Palette Log          | createdAt 기반 월별 기록 화면 추가      |
...
# v1.2.0 — Sticker Frame UI

## Goal

Pieces of Me 페이지의 컬러칩을 빈티지 라벨 또는 스티커 형태로 표현하여 기본 팔레트 페이지와 차별화한다.
...
컬러 데이터에 createdAt이 있어도 v1~v3에서는 날짜별 기록 흐름을 시각적으로 확인할 수 없다.
...
> v1.0에서는 React 기본기로 핵심 CRUD를 완성하고, 이후 TypeScript, Zustand, 스타일 시스템, localStorage, 날짜 기반 기록 UI, 접근성, Backend 연동을 단계적으로 적용하며 프로젝트의 안정성, 유지보수성, 사용성을 개선했습니다.
...
- 달력 UI에서 같은 날짜의 여러 데이터를 어떻게 표현했는가?
