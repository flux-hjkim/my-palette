...
### Editor Page 카테고리별 문구 개선

- 카테고리별로 Editor Page의 안내 문구와 placeholder를 다르게 표시
- `from 카테고리 대표 컬러명` 대신 `from 카테고리명`으로 표시
- 카테고리명 텍스트에는 해당 카테고리 대표 컬러 적용
- 사용자가 현재 어떤 카테고리에 색을 추가하는지 더 명확하게 인식하도록 개선

### Editor Color Preview 개선

- 컬러 preview 박스 안에 선택한 컬러 이름 표시
- 컬러만 보이는 상태보다 선택한 색의 정체성을 더 명확하게 표현
- 밝은 컬러에서도 글자가 보이도록 텍스트 색상 처리 검토
- 추후 빈티지 아이콘 또는 장식 요소 추가 여부 검토

### Editor Keyword Layout 개선
